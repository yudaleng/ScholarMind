...
def get_journal_metrics(journal_name: str, api_key: Optional[str] = None, 
    ...
    try:
        ...
        if data["code"] == 200 and "data" in data:
            ...
            # 处理自定义数据集 (customRank)
            if "customRank" in api_data and any(m.startswith("custom_") for m in metrics_to_fetch):
                if "rankInfo" in api_data["customRank"] and "rank" in api_data["customRank"]:
                    custom_rank_info = {item["uuid"]: item for item in api_data["customRank"]["rankInfo"]}
